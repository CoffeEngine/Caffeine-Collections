...
static void print_container(cff_container* container) {
	caffeine_container_foreach(container, print_vec3, INI_LEN);
	printf("\n");
}
...
static void container_arange(cff_container* container, int start, int end) {
	for (int i = start, j = 0; i < end; i++,j++)
	{
		...
		caffeine_container_set(container, j, (uintptr_t)(&data), INI_LEN);
	}
}

#pragma region CALLBACKS

static cff_cmp_e find_cmp(void* ptr_a, void* ptr_b, uint64_t data_size) {
	vec3* a = (vec3*)ptr_a;
	vec3* b = (vec3*)ptr_b;
...
TESTDEF(container_create) {
	SKIP_ON_ERR(caffeine_container_create(container, DATA_SIZE, INI_LEN, NULL));
...
TESTDEF(container_set) {
	...
	caffeine_container_set(container, (uint64_t)index, (uintptr_t)(&data), INI_LEN);
...
TESTDEF(container_get) {
	...
	caffeine_container_get(container, (uint64_t)index, (uintptr_t)(&get_data), INI_LEN);
...
TESTDEF(container_desloc) {
	...
	int index = 3;
	int step = 2;
	caffeine_container_desloc(container, (uint64_t)index, (int64_t)step, INI_LEN);
	vec3 get_data = {0};
	caffeine_container_get(container, (uint64_t)index + step, (uintptr_t)(&get_data), INI_LEN);
...
TESTDEF(container_resize) {
	SKIP_ON_ERR(caffeine_container_resize(container, INI_LEN * 2, NULL));
...
TESTDEF(container_insert) {
	...
	vec3 get_data = {0};
	int insert_index = 5;
	caffeine_container_insert(container, insert_index,(uintptr_t)(&data), INI_LEN);
	caffeine_container_get(container, insert_index, (uintptr_t)(&get_data), INI_LEN);
...
TESTDEF(container_remove) {
	...
	vec3 get_data = {0};
	uint8_t res = caffeine_container_remove(container, index_remove, INI_LEN);
	
	munit_assert_false(index_remove != (INI_LEN-1) && res == 0);
	munit_assert_false(index_remove == (INI_LEN-1) && res == 1);

	if (res) {
		caffeine_container_get(container, index_remove, (uintptr_t)(&get_data), INI_LEN);
...
TESTDEF(container_copy) {
	...
	uint64_t n_len = 0;
	SKIP_ON_ERR(caffeine_container_copy(container, &dest, start, end, INI_LEN, &n_len, NULL));
	for (uint64_t i = start, j = 0; i < end; i++, j++)
	{
		vec3 a = { 0 };
		vec3 b = { 0 };
		caffeine_container_get(container, i, (uintptr_t)(&a), INI_LEN);
		caffeine_container_get(&dest, j, (uintptr_t)(&b), n_len);
		assert_vec3(a, b);
	}
	caffeine_container_free(&dest, NULL);
	return MUNIT_OK;
}
...
TESTDEF(container_clone) {
	...
	SKIP_ON_ERR(caffeine_container_clone(container, &dest, INI_LEN, NULL));
	...
	for (size_t i = 0; i < INI_LEN; i++)
	{
		...
		vec3 b = { 0 };
		caffeine_container_get(container, i, (uintptr_t)(&a), INI_LEN);
		caffeine_container_get(&dest, i, (uintptr_t)(&b), INI_LEN);
		assert_vec3(a, b);
	}
	caffeine_container_free(&dest, NULL);
	return MUNIT_OK;
...
TESTDEF(container_reverse) {
	...
	SKIP_ON_ERR(caffeine_container_reverse(container, INI_LEN));
	for (int i = 0; i < INI_LEN; i++)
	{
		vec3 out = { 0 };
		vec3 data = { .x = i * 3,.y = i * 5,.z = i * 7 };
		caffeine_container_get(container, (uint64_t)(INI_LEN - i - 1), (uintptr_t)(&out), INI_LEN);
...
TESTDEF(container_fill) {
	...
	caffeine_container_fill(container, (uintptr_t)(&data), INI_LEN);
	for (size_t i = 0; i < INI_LEN; i++)
	{
		vec3 out = { 0 };
		caffeine_container_get(container, i, (uintptr_t)(&out), INI_LEN);
		assert_vec3(data, out);
	}
	return MUNIT_OK;
}
...
TESTDEF(container_join) {
	...
	SKIP_ON_ERR(caffeine_container_create(&other, DATA_SIZE, INI_LEN / 2, NULL));

	container_arange(container, 0, (int)(INI_LEN / 2));
	container_arange(&other,(int)( INI_LEN / 2),(int)(INI_LEN));

	SKIP_ON_ERR(caffeine_container_join(container, &other, INI_LEN / 2, INI_LEN, INI_LEN / 2, NULL));
...
TESTDEF(container_filter) {
	...
	uint64_t f_len = 0;
	SKIP_ON_ERR(caffeine_container_filter(container, filter_even, &other, INI_LEN, &f_len, NULL));

	for (size_t i = 0; i < f_len; i++)
	{
		vec3 out = { 0 };
		caffeine_container_get(&other, i, (uintptr_t)(&out), f_len);
		munit_assert(out.x % 2 == 0);
	}
	caffeine_container_free(&other, NULL);
	return MUNIT_OK;
}
...
TESTDEF(container_map) {
	...
	container_arange(container, 0, (int)INI_LEN);
	SKIP_ON_ERR(caffeine_container_map(container, map_vec, &other, sizeof(int), INI_LEN, NULL));
	...
	for (size_t i = 0; i < INI_LEN; i++)
	{
		vec3 data = { 0 };
		int sum = 0;
		caffeine_container_get(container, i, (uintptr_t)(&data), INI_LEN);
		caffeine_container_get(&other, (uint64_t)i, (uintptr_t)&sum, INI_LEN);
...
TESTDEF(container_foreach) {
	container_arange(container, 0, (int)INI_LEN);
	caffeine_container_foreach(container, foreach_func, INI_LEN);
	return MUNIT_OK;
}
...
TESTDEF(container_sort) {
	...
	container_arange(container, 0, (int)INI_LEN);
	SKIP_ON_ERR(caffeine_container_clone(container, &other, INI_LEN, NULL));
	SKIP_ON_ERR(caffeine_container_reverse(&other, INI_LEN));
	SKIP_ON_ERR(caffeine_container_sort(&other, find_cmp, INI_LEN));

	uint8_t eq = caffeine_container_equal(container, &other, INI_LEN);
	caffeine_container_free(&other, NULL);
...
TESTDEF(container_free) {
	caffeine_container_free(container, NULL);
...
TESTDEF(container_equal) {
	cff_container other;
	SKIP_ON_ERR(caffeine_container_create(&other, DATA_SIZE, INI_LEN, NULL));
...
TESTDEF(container_find) {
	...
	uint64_t found = 0;
	uint8_t f = caffeine_container_find(container, (uintptr_t)(&data), &found, INI_LEN);
...
TESTDEF(container_find_cmp) {
	...
	vec3 data = { .x = index * 3,.y = index * 5,.z = index * 7 };
	uint64_t found = 0;
	caffeine_container_find_cmp(container, (uintptr_t)(&data), &found, find_cmp, INI_LEN);
	munit_assert(found == index);
	return MUNIT_OK;
}
...
TESTDEF(container_count) {
	...
	for (int i = 0; i < INI_LEN; i++)
	{
		...
		cc += i % 2;
		caffeine_container_set(container, i, (uintptr_t)(&data), INI_LEN);
	}
	vec3 data = { .x = 1,.y = 1,.z = 1 };
	uint64_t count = caffeine_container_count(container, (uintptr_t)(&data), INI_LEN);
...
TESTDEF(container_count_cmp) {
	...
	vec3 data = { .x = 1,.y = 1,.z = 1 };
	uint64_t count = caffeine_container_count_cmp(container, (uintptr_t)(&data), find_cmp, INI_LEN);
	munit_assert(cc == count);
	return MUNIT_OK;
}
...
TESTDEF(container_any) {
	...
	vec3 data = { .x = 1,.y = 1,.z = 1 };
	uint8_t exist = caffeine_container_any(container, (uintptr_t)(&data), INI_LEN);
	munit_assert(exist);
	return MUNIT_OK;
}
...
TESTDEF(container_any_cmp) {
	...
	vec3 data = { .x = 1,.y = 1,.z = 1 };
	uint8_t exist = caffeine_container_any_cmp(container, (uintptr_t)(&data), find_cmp, INI_LEN);
	munit_assert(exist);
	return MUNIT_OK;
}
...
TESTDEF(container_all) {
	...
	vec3 data = { .x = 1,.y = 1,.z = 1 };
	uint8_t exist = caffeine_container_all(container, (uintptr_t)(&data), INI_LEN);
...
TESTDEF(container_all_cmp) {
	...
	vec3 data = { .x = 1,.y = 1,.z = 1 };
	uint8_t exist = caffeine_container_all_cmp(container, (uintptr_t)(&data), find_cmp, INI_LEN);
	munit_assert_false(exist);
	return MUNIT_OK;
}
...
static void* test_setup(const MunitParameter params[], void* user_data) {
	...
	caffeine_container_create(container, DATA_SIZE, INI_LEN, NULL);
	return container;
}
...
static void test_tear_down(void* fixture) {
	caffeine_container_free(fixture, NULL);
	free(fixture);
}
